  ...
  try {
    ...
    if (req.body.avatar !== undefined) {
      ...
      if (
        ...
        !/^data:image\/(png|jpe?g|webp|gif);base64,/.test(base64Data)
      ) {
      ...
      try {
        ...
        const timeoutPromise = new Promise((_, reject) =>
          setTimeout(() => reject(new Error("ImageKit upload timeout - please try again")), 30000)
        );
        ...
        // Return response with image upload error but continue with other updates
        return res.status(400).json({
          status: "Fail",
          message: `Image upload failed: ${imageError.message || "Please check your internet connection and try again"}`,
        });
      }
    }
